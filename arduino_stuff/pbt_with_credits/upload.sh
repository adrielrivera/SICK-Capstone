#!/bin/bash
# Arduino Upload Script for PBT Sensor with GPIO Control + Credit Tracking

ARDUINO_DIR="$(dirname "$0")"
SKETCH_NAME="pbt_with_credits.ino"
FQBN="arduino:avr:uno" # Fully Qualified Board Name for Arduino Uno

echo "=========================================="
echo "PBT Sensor with GPIO + Credit Tracking"
echo "Arduino Upload Script"
echo "=========================================="

# Find Arduino port (prioritize ttyUSB0 for PBT Arduino)
ARDUINO_PORT=$(ls /dev/ttyUSB0 /dev/ttyACM* 2>/dev/null | head -n 1)
...
echo "Credit tracking: 2 PBT hits = 1 credit deducted"
echo "=========================================="
